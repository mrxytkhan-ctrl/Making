...
void *direct_attack(void *arg) {
    ...
    if (PACKET_SIZE <= 40) {
        ...
        optimal_burst = 500 + (fast_rand(&seed) % 250);
    } else {
        optimal_burst = 600 + (fast_rand(&seed) % 300);
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("🧵 Threads: %d (100-220)\n", THREAD_COUNT);
    printf("⚡ Direct Attack Power: %d%%\n", DIRECT_ATTACK_POWER);
    printf("🔥 Warm-up: 200%% → 1400%% in 5s\n");
    printf("🔧 Optimized for maximum impact\n");
    printf("===============================================\n");
