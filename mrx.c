...
void *show_stats(void *arg) {
    ...
    time_t last_time = 0;
    unsigned long long highest_pps = 0;
    ...
    while (!atomic_load(&stop_all)) {
        ...
        double mbps = (byte_diff * 8.0) / (1024.0 * 1024.0 * elapsed);
        
        if (pps > highest_pps) {
            highest_pps = pps;
        }
        
        printf("PPS: %.0f | Mbps: %.1f | Peak: %.0f | Total: %llu\n", 
               pps, mbps, (double)highest_pps, current_packets);
...
int main(int argc, char *argv[]) {
    ...
    if (pthread_create(&heavy_thread, NULL, heavy_flood, &heavy_thread_id) != 0) {
        printf("Error: Failed to create heavy flood thread\n");
        return 1;
    }
    ...
    printf("Started: %d threads + 1 heavy thread\n\n", threads_created);
    ...
    if (ATTACK_DURATION > 0) {
        ...
        double avg_mbps = (final_bytes * 8.0) / (ATTACK_DURATION * 1024.0 * 1024.0);
        
        printf("\n========== ATTACK COMPLETE ==========\n");
        printf("Total Packets Sent: %llu\n", final_packets);
        printf("Average Packets/Second: %.0f\n", avg_pps);
