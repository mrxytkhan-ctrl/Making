...
#define WARMUP_BURST_MULTIPLIER 1      // 1x (normal) - REDUCED
#define WARMUP_SLEEP_DIVIDER 1         // 1x (normal) - REDUCED
#define MAIN_BURST_MULTIPLIER 1.2      // 1.2x (120%) - INCREASED
#define MAIN_SLEEP_DIVIDER 1.2         // 1.2x (120% faster) - INCREASED
        ...
        if (atomic_load(&should_attack)) {
            ...
            if (!atomic_load(&warmup_complete)) {
                if (!warmup_displayed && pps > 0) {
                    printf("📡 Warm-up packets flowing...\n");
                    warmup_displayed = 1;
                }
            } else {
