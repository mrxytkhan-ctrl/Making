...
_Atomic int global_packet_size = 8;
...
int get_base_burst(int packet_size) {
    if (packet_size <= 48) return 130;
    if (packet_size <= 56) return 120;
    if (packet_size <= 64) return 110;
    if (packet_size <= 72) return 100;
    return 90;
}
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_elapsed % 5 == 0 && current_elapsed > last_rotation) {
            ...
            if (thread_idx % 5 == 0) {
                ...
                int len;
                if (phase == 0) {
                    static int s1[] = {8,12,14,16,18,20};
                    sizes = s1; len = 6;
                } else if (phase == 1) {
                    static int s2[] = {16,18,20,22,24,26};
                    sizes = s2; len = 6;
                } else {
                    static int s3[] = {22,24,26,28,30,32,34};
                    sizes = s3; len = 7;
                }
...
int main(int argc, char *argv[]) {
    ...
    if (PACKET_SIZE <= 34) {
        printf("💡 Target PPS: 720K - 880K\n");
    } else {
        printf("💡 Target PPS: 680K - 800K\n");
    }
    printf("\n");
